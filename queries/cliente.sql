...
DROP FUNCTION IF EXISTS getClientsUserName() CASCADE;
CREATE OR REPLACE FUNCTION getClientsUserName()
RETURNS SETOF client_vdb.username%TYPE AS $$
BEGIN
...
-- PARA FACILITAR A SIMULAÇÃO
DROP FUNCTION IF EXISTS getValCli() CASCADE;
CREATE OR REPLACE FUNCTION getValCli()
RETURNS TABLE(idcli client_vdb.idCli%TYPE,username client_vdb.username%TYPE) AS $$
BEGIN
...
-- add histórico
DROP FUNCTION IF EXISTS add_history_cli(client_history.idCli%TYPE,client_history.info%TYPE,CHAR) CASCADE;
CREATE OR REPLACE FUNCTION add_history_cli(client_history.idCli%TYPE,client_history.info%TYPE,CHAR)
RETURNS VOID AS $$
BEGIN
...
DROP FUNCTION IF EXISTS getHistsCli(CHAR) CASCADE;
CREATE OR REPLACE FUNCTION getHistsCli(CHAR)
...
DROP FUNCTION IF EXISTS getHistCli(client_history.idCli%TYPE, CHAR) CASCADE;
CREATE OR REPLACE FUNCTION getHistCli(client_history.idCli%TYPE, CHAR)
RETURNS SETOF client_history AS $$
BEGIN
