...
DROP FUNCTION IF EXISTS upd_bike(bike.idBike%TYPE,VARCHAR(30),bike.state%TYPE) CASCADE;
CREATE OR REPLACE FUNCTION upd_bike(bike.idBike%TYPE, VARCHAR(30),bike.state%TYPE)
RETURNS VOID AS $$
BEGIN
	UPDATE bike SET
		name = $2,
...
RETURNS TABLE(idBike bike.idBike%TYPE,name bike.name%TYPE) AS $$
BEGIN
...
RETURNS TABLE(idBike bike.idBike%TYPE,name bike.name%TYPE,slot bike.slot%TYPE,state bike.state%TYPE) AS $$
BEGIN
